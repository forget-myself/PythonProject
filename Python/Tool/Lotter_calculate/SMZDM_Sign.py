...
def PrintfTime(starttime,str):
    timestr2= (datetime.datetime.now() - starttime).microseconds
    print(str)
    print(timestr2)

def SMZDMTrySign(commonWebrowser):
    try:
        starttime = datetime.datetime.now()
        # 初始化浏览器
        browser = commonWebrowser # webdriver.Chrome()
        # 加载张大妈首页
        browser.get('http://www.smzdm.com')
        PrintfTime(starttime,"打开网页")
       # time.sleep(5)
        # 定位登录按钮，并模拟点击
        button_login = browser.find_element_by_class_name('J_login_trigger')
        button_login.click()
        PrintfTime(starttime, "点击登录 ")
      #  time.sleep(1)
        # 点击登录按钮后，在页面生成一个包含登录框的iframe，切换至该iframe
        browser.switch_to.frame('J_login_iframe')
        PrintfTime(starttime, "生成一个包含登录框的iframe ")
        # 定位用户名和密码输入框，并键入用户名密码
        username = browser.find_element_by_id('username')
        password = browser.find_element_by_id('password')
        username.send_keys('1561033845@qq.com')
        password.send_keys('123455')
        PrintfTime(starttime, "定位用户名和密码输入框 ")

        # 定位登录按钮，模拟点击进行登录
        button_submit = browser.find_element_by_id('login_submit')
        button_submit.click()
        PrintfTime(starttime, "模拟点击进行登录 ")

        count = 6  # 最多识别6次
        time.sleep(2)
        driver = browser
        # 等待滑动按钮加载完成
        element = main_check_slider(browser)
        while count > 0:
